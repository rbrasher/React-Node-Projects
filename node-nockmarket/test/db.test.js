...
suite('database', function() {
    ...
    test('insertOne should insert a transaction', function(done) {
        ...
        db.insertOne('transactions', id, function(err, order) {
            should.not.exist(err);
            should.exist(order._id); // main test to check for existence of an _id field
            insertedOrder = order;
            done();
        });
    });
    ...
    test('findOne should find a single transaction', function(done) {
        ...
        db.findOne('transactions', id, function(err, done) {
            should.not.exist(err);
